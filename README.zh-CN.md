...
- **跨发行版 arm64**:`/armpkgs <包名>` 查该包在 **Gentoo、Debian、Ubuntu、Fedora、Arch Linux ARM** 上的 arm64 支持(各走自己的按架构 API)。特别适合 Gentoo 还没给某包 keyword arm64、但别的发行版已经有的情况 —— 这通常说明它能用,可 `ACCEPT_KEYWORDS="~arm64"` 强制开启自行编译。
