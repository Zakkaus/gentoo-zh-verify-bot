...
- **跨发行版查包**:`/distro <包名>` 一条消息显示一个包在 **AUR、Arch、Debian、Ubuntu、Nixpkgs、openSUSE、Fedora** 各自的当前版本(走 [Repology](https://repology.org) API)。每个发行版都链到其软件包页面;查不到精确匹配时会给出最接近的跨发行版包建议。支持 `rich_messages` / `/rich` 富文本开关(对齐 `/pkg`、`/use`)。
