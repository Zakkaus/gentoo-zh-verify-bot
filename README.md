...
- **Cross-distro search:** `/distro <pkg>` shows a package's current version across **AUR, Arch, Debian, Ubuntu, Nixpkgs, openSUSE and Fedora** in one message, via the [Repology](https://repology.org) API. Each distro links to its package page; an unmatched query suggests the closest cross-distro packages. Honors the `rich_messages` / `/rich` toggle like `/pkg` and `/use`.
