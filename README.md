...
- **Cross-distro arm64:** `/armpkgs <pkg>` checks arm64 support across **Gentoo, Debian, Ubuntu, Fedora and Arch Linux ARM** (each via its own per-arch API). Handy when Gentoo hasn't keyworded a package for arm64 but other distros ship it — a strong hint it builds fine with `ACCEPT_KEYWORDS="~arm64"`.
