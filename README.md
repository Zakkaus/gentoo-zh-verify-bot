...
- **Moderation commands** (reply to a message, admins only): `/sb` and `/ban` = delete + ban the user for the **configured duration** (`/bantime`, default permanent), `/warn` = strike a user (auto-kick after `warn_limit`, default 3 — counts persist across restarts), `/clearwarn` = clear a user's strikes.
