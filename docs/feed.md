...
Bugs are posted in ascending ID order. A successful send advances the cursor and records the Telegram message ID. A failed send stops at that bug and does not advance across it; later polls retry. At most 100 bugs are processed per destination cycle, so a larger backlog drains as contiguous batches.
...
Fetch, parse, and transient Telegram failures leave the relevant cursor or tracked state unchanged. A normal bug post’s permanent-error classification is not used, so even a deterministic rejection is retried later. News alone skips errors classified as permanently item-specific.
