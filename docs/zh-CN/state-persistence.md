...
**实现位置：**`internal/store` 包；`internal/store/json.go` 中的 `Load`、`Save`、`Write`、`writeLocked` 和 `ReclaimTemps`；`main` 包；`cmd/gentoo-zh-verify-bot/main.go` 中的 `loadRuntimeState`。
...
文件不存在是正常首次状态。JSON 解码失败时，程序记录日志，并尝试把原文件改名为 `<name>.corrupt`；代码没有这些备份的保留或清理策略。现有文件不可读时，`Load` 返回专用错误，供核心调用者保留文件并停用后续写入。未知 JSON 字段可以读入，但后续重写时会丢失。
...
- JSON 损坏：尽量把原文件改名为 `.corrupt`；进程从 baseline 开始且仍可写。后续提交创建新的版本 2 状态。旧 owner 和注册信息在人工恢复前不可用。
...
文件缺失或损坏时不恢复待验证记录；损坏输入备份后，后续写入仍启用。文件不可读时不恢复记录，并清空服务中的路径，避免本进程覆盖可恢复原件。后续保存失败时，存储层记录日志，当前申请仍在内存中处理，但重启后可能丢失。
...
每个文件缺失或损坏时均从空值开始。每个不可读文件只会停用本进程对自身路径的后续写入。写入失败时，当前内存中的失败记录、统计或 heartbeat 仍有效，调用者不再处理错误。Heartbeat 不可用会失去长故障依据，但不阻止恢复 `pending.json`。
...
文件缺失或损坏时从空状态开始，下一次成功请求只建立基线，不补发历史。Feed 读取到不可读错误后不会停用写入，而是继续尝试新的原子写入。保存失败不阻止投递，也不回滚内存游标。重启后只能恢复最后持久成功的游标和跟踪状态。
...
`<name>.corrupt` 是解码失败后保留的输入，不会自动读取。`.<name>.tmp-*` 是中断的原子写入，匹配存储模式时会在启动阶段清理。

## 不跨重启保存的状态

**实现位置：**`internal/verify` 包；`internal/verify/state.go` 中的 `(*Service).Stats`；`internal/panel` 包；`internal/panel/session.go` 中的 `(*Panel).newSettingsSession`；`main` 包；`cmd/gentoo-zh-verify-bot/main.go` 中的 `main`。
