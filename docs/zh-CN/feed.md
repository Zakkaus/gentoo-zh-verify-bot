...
Bug 按 ID 升序发送。发送成功后推进游标并记录 Telegram 消息 ID。发送失败时停在该 Bug，不跨过它推进；后续周期重试。每个目标每轮最多处理 100 条，因此更大的积压按连续批次消化。
...
成功发送且消息 ID 非零的 Bug 都会跟踪。`status|resolution` 任一部分变化时，程序编辑原消息，包括确认、解决、重新打开和修改 resolution。保存状态从 `UNCONFIRMED` 变为另一种未解决且应通知的状态时，还会发送一条非静默回复。直接变为已解决时只编辑原消息。该回复发送失败时，旧状态保持不变，以便后续周期重新编辑并发送；连续失败十次后放弃回复并推进状态。
...
请求、解析和暂时性 Telegram 失败不会推进对应游标或跟踪状态。普通 Bug 发布不使用永久错误分类，因此确定性拒绝也会在后续周期重试。只有新闻会跳过被分类为单条永久错误的项目。
