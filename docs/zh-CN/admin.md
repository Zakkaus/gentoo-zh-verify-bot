...
批准操作先占用待验证记录，再调用 Telegram，因此超时不能并发处理同一记录。批准成功后删除待验证记录和旧失败次数。批准失败时重新开放记录、启动重试计时，并通知运维人员。

封禁操作会取走待验证记录，尽力拒绝入群申请，再按群组当前封禁时长执行带消息撤回的封禁。程序在网络操作前先确认回调。封禁失败会发送运维告警，但待验证记录已经处理，群内提示也会删除。重复点击或申请者已经完成处理时，回调返回已处理。
...
- 运行参数：验证开关、优先私聊验证（默认启用）、模式、姓名隐藏、封禁时长、查询自动删除及 TTL、群组语言；
- 列表：频道身份白名单、受信任成员群组、已知或支持群组；
- 验证参数：30 至 1,800 秒超时、最大失败次数或关闭、冷却时间或关闭，以及全局 DM 查询频率；
- 必加频道：选择频道、设置或清除私有邀请、停用频道检查。

`dm_first` 的内置基线为 `true`。运行参数开关按照面板显示的 revision 提交按群稀疏覆盖值；恢复为 `true` 时，程序删除与基线相同的覆盖值。其他提交同时修改该群组时，面板按相邻运行参数开关的相同规则结束会话并报告冲突。
