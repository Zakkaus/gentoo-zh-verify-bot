...
Bot-caused reasons—failed challenge delivery, failed approval or decline retry, a lapsed deadline during a short restart, and recovery windows—may still decline the pending join request when their grace timer ends, but do not record a strike. Outage-specific deferral is covered in [Outage and recovery](outage-recovery.md).
